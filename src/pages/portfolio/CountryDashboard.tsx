...
import {
  aggregateKPIs, findCountryGroup, findIndustry, projectsForCountry, regions,
                ...
                return (
                  <div key={ind.key} className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-ink-100 text-ink-600"><Icon size={16} strokeWidth={1.8} /></div>
                    ...
                    <div className="mt-2.5 flex flex-wrap gap-1.5">
                      {industryRegions.map((r) => (
                        <Link key={r.id} to={`/portfolio/${ind.key}/${r.id}`} className="rounded-full border border-ink-200 px-2.5 py-1 text-[11.5px] font-medium text-ink-600 hover:border-accent-500 hover:text-accent-700">
