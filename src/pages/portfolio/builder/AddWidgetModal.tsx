      ...
      {step === 3 && (
        <div>
          ...
          <div className="mb-5 flex flex-wrap gap-1.5">
            {comparisons.map((c) => (
              <button key={c.key} onClick={() => setComparison(c.key)} className={cn("rounded-full border px-3 py-1.5 text-[12px] font-medium", comparison === c.key ? "border-accent-600 bg-accent-50 text-accent-700" : "border-ink-200 text-ink-600 hover:border-ink-300")}>{c.label}</button>
            ))}
          </div>
