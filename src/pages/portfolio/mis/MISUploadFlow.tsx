...
export default function MISUploadFlow({
  ...
  return (
    <Modal open={open} onClose={close} title="Upload MIS report" sub={projectName} width="640px">
      {step === "upload" && (
        <div>
          <button
            ...
            className="flex w-full flex-col items-center justify-center rounded-xl border-2 border-dashed border-ink-200 bg-white py-10 text-center transition-colors hover:border-accent-500/50 hover:bg-accent-50/30"
          >
      ...
      {step === "done" && (
        <div className="flex flex-col items-center justify-center py-14 text-center">
          <div className="mb-3 flex h-11 w-11 items-center justify-center rounded-xl bg-pos-50 text-pos-600"><Check size={20} /></div>
