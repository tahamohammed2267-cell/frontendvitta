              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {driverSchemaLabels[compatibleDriverKind].map((label) => (
                    <span key={label} className="rounded-full border border-ink-200 px-3 py-1.5 text-[12px] font-medium text-ink-400" title="Business driver metrics are shown for reference — select from the categories below for numeric comparison.">
                      {label}
                    </span>
                  ))}
                </div>
      ...
      {step === 4 && (
        <div>
          ...
          <div className="mb-4 flex gap-1.5">
            {timeRanges.map((t) => (
              <button key={t} onClick={() => setTimeRange(t)} className={cn("rounded-full border px-3 py-1.5 text-[12px] font-medium", timeRange === t ? "border-ink-900 bg-ink-900 text-white" : "border-ink-200 text-ink-600 hover:border-ink-300")}>{t}</button>
            ))}
          </div>
          {showFilters && (
            <>
              ...
              <div className="mb-5 flex flex-wrap gap-1.5">
                {statuses.map((s) => (
                  <button key={s} onClick={() => toggleStatus(s)} className={cn("rounded-full border px-3 py-1.5 text-[12px] font-medium", statusFilter.includes(s) ? "border-accent-600 bg-accent-50 text-accent-700" : "border-ink-200 text-ink-600 hover:border-ink-300")}>{s}</button>
                ))}
              </div>
            </>
          )}
