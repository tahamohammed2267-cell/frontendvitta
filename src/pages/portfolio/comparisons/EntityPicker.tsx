...
export function EntityChip({ entity, onRemove }: { entity: ComparableEntity; onRemove: () => void }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-ink-200 bg-white py-1 pl-1 pr-2 text-[12px] font-medium text-ink-700">
      <Badge tone={kindTone[entity.kind]}>{kindLabel[entity.kind]}</Badge>
      {entity.label}
      <button onClick={onRemove} className="rounded-full p-0.5 text-ink-400 hover:bg-ink-100 hover:text-crit-600"><X size={11} /></button>
    </span>
  );
}
