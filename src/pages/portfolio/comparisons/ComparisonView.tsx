...
import {
  getComparisonEntityMetricValue, getEntityMetricSeries, metricCategories, metricCategoryLabels, metricLabels, metricUnits, type MetricCategory,
...
const palette = ["#2563eb", "#059669", "#d97706", "#dc2626", "#8a93a6", "#12161f"];
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-6 py-6">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-1.5 fade-up">
        ...
        <button onClick={() => setAddEntityOpen(true)} className="flex items-center gap-1 rounded-full border border-dashed border-ink-300 px-2.5 py-1 text-[12px] font-medium text-ink-500 hover:border-accent-500 hover:text-accent-700">
        ...
        <div className="mb-6 grid gap-4 fade-up" style={{ gridTemplateColumns: `repeat(${Math.min(c.entities.length, 4)}, minmax(0,1fr))` }}>
          {c.entities.map((e) => (
            <Card key={e.id}>
              <Stat
                label={e.label}
                value={`${getComparisonEntityMetricValue(headlineMetric, e).toLocaleString()}${metricUnits[headlineMetric]}`}
      ...
      <div className="mb-4 flex items-center gap-1.5 fade-up">
        ...
        {timeRanges.map((t) => (
          <button key={t} onClick={() => setTimeRange(t)} className={cn("rounded-full border px-3 py-1.5 text-[12px] font-medium", c.timeRange === t ? "border-ink-900 bg-ink-900 text-white" : "border-ink-200 text-ink-600 hover:border-ink-300")}>{t}</button>
        ))}
      </div>
                ...
                <tr key={m}>
                  ...
                  {c.entities.map((e) => (
                    <td key={e.id} className="num px-4 py-3 text-[12.5px] font-semibold">
                      {getComparisonEntityMetricValue(m, e).toLocaleString()}{metricUnits[m]}
                    </td>
                  ))}
                </tr>
              ))}
